...
import (
	"bytes"
	"fmt"
	"github.com/filedag-project/filedag-storage/http/objectstore/utils/signature"
	"net/http"
	"net/url"
	"testing"
)
...
func TestS3ApiServer_PutObjectHandler(t *testing.T) {
	...
	reqPutBucket := signature.MustNewSignedV4Request(http.MethodPut, bucketName, 0, nil, "s3", DefaultTestAccessKey, DefaultTestSecretKey, t)
	fmt.Println("putbucket:", reqTest(reqPutBucket).Body.String())
	// Iterating over the cases, fetching the object validating the response.
	for i, testCase := range testCases {
		req := signature.MustNewSignedV4Request(http.MethodPut, testCase.bucketName+testCase.objectName, int64(len(r1)), bytes.NewReader(testCase.data), "s3", testCase.accessKey, testCase.secretKey, t)
...
func TestS3ApiServer_GetObjectHandler(t *testing.T) {
	...
	r1 := "1234567"
	reqputObject := signature.MustNewSignedV4Request(http.MethodPut, bucketName+objectName, int64(len(r1)), bytes.NewReader([]byte(r1)), "s3", DefaultTestAccessKey, DefaultTestSecretKey, t)
	// Add test case specific headers to the request.
	reqTest(reqputObject)
	// Iterating over the cases, fetching the object validating the response.
	for i, testCase := range testCases {
		req := signature.MustNewSignedV4Request(http.MethodGet, testCase.bucketName+testCase.objectName, 0, nil, "s3", testCase.accessKey, testCase.secretKey, t)
		// Add test case specific headers to the request.
		result := reqTest(req)
		if result.Code != testCase.expectedRespStatus {
			t.Fatalf("Case %d: Expected the response status to be `%d`, but instead found `%d`", i+1, testCase.expectedRespStatus, result.Code)
...
func TestS3ApiServer_HeadObjectHandler(t *testing.T) {
	...
	// Iterating over the cases, fetching the object validating the response.
	for i, testCase := range testCases {
		req := signature.MustNewSignedV4Request(http.MethodHead, testCase.bucketName+testCase.objectName, 0, nil, "s3", testCase.accessKey, testCase.secretKey, t)
		// Add test case specific headers to the request.
		result := reqTest(req)
		if result.Code != testCase.expectedRespStatus {
			t.Fatalf("Case %d: Expected the response status to be `%d`, but instead found `%d`", i+1, testCase.expectedRespStatus, result.Code)
...
func TestS3ApiServer_DeleteObjectHandler(t *testing.T) {
	...
	// Iterating over the cases, fetching the object validating the response.
	for i, testCase := range testCases {
		req := signature.MustNewSignedV4Request(http.MethodDelete, testCase.bucketName+testCase.objectName, 0, nil, "s3", testCase.accessKey, testCase.secretKey, t)
		// Add test case specific headers to the request.
		result := reqTest(req)
		if result.Code != testCase.expectedRespStatus {
			t.Fatalf("Case %d: Expected the response status to be `%d`, but instead found `%d`", i+1, testCase.expectedRespStatus, result.Code)
...
func TestS3ApiServer_CopyObjectHandler(t *testing.T) {
	...
	// Iterating over the cases, fetching the object validating the response.
	for i, testCase := range testCases {
		req := signature.MustNewSignedV4Request(http.MethodPut, testCase.dstbucketName+testCase.dstobjectName, 0, nil, "s3", testCase.accessKey, testCase.secretKey, t)
...
func TestS3ApiServer_ListObjectsV2Handler(t *testing.T) {
	...
	// Iterating over the cases, fetching the object validating the response.
	for i, testCase := range testCases {
		req := signature.MustNewSignedV4Request(http.MethodGet, testCase.bucketName+"?list-type=2", 0, nil, "s3", testCase.accessKey, testCase.secretKey, t)
		result := reqTest(req)
		if result.Code != testCase.expectedRespStatus {
			t.Fatalf("Case %d: Expected the response status to be `%d`, but instead found `%d`", i+1, testCase.expectedRespStatus, result.Code)
		}
...
func TestWholeProcess(t *testing.T) {
	...
	reqPutUser := signature.MustNewSignedV4Request(http.MethodPost, "/admin/v1/add-user"+"?accessKey="+userName+"&secretKey="+secret, 0, nil, "s3", DefaultTestAccessKey, DefaultTestSecretKey, t)
	fmt.Println(reqTest(reqPutUser).Body.String())
	reqPutBucket := signature.MustNewSignedV4Request(http.MethodPut, bucketName, 0, nil, "s3", userName, secret, t)
	fmt.Println("putbucket:", reqTest(reqPutBucket).Body.String())
	r1 := "1234567"
	reqputObject := signature.MustNewSignedV4Request(http.MethodPut, bucketName+objectName, int64(len(r1)), bytes.NewReader([]byte(r1)), "s3", userName, secret, t)
	// Add test case specific headers to the request.
	reqTest(reqputObject)
	req := signature.MustNewSignedV4Request(http.MethodGet, bucketName+objectName, 0, nil, "s3", userName, secret, t)
...
func TestS3ApiServer_PutObjectHandler2(t *testing.T) {
	...
	r2 := "123456"
	reqputObject2 := signature.MustNewSignedV4Request(http.MethodPut, bucketName+objectName, int64(len(r1)), bytes.NewReader([]byte(r2)), "s3", DefaultTestAccessKey, DefaultTestSecretKey, t)
	// Add test case specific headers to the request.
	re = reqTest(reqputObject2)
	if re.Code != http.StatusOK {
		t.Fatalf("Case %d: Expected the response status to be `%d`, but instead found `%d`", 1, http.StatusOK, re.Code)
	}
	req := signature.MustNewSignedV4Request(http.MethodGet, bucketName+objectName, 0, nil, "s3", DefaultTestAccessKey, DefaultTestSecretKey, t)
	re = reqTest(req)
	if re.Code != http.StatusOK {
		t.Fatalf("Case %d: Expected the response status to be `%d`, but instead found `%d`", 1, http.StatusOK, re.Code)
	}
