...
func TestS3ApiServer_PutBucketHandler(t *testing.T) {
	...
	for i, testCase := range testCases {
		...
		reqPutBucket := signature.MustNewSignedV4Request(http.MethodPut, testCase.bucketName, 0, nil, "s3", testCase.accessKey, testCase.secretKey, t)
...
func TestS3ApiServer_HeadBucketHandler(t *testing.T) {
	...
	for i, testCase := range testCases {
		// mock an HTTP request
		reqPutBucket := signature.MustNewSignedV4Request(http.MethodPut, bucketName, 0, nil, "s3", testCase.accessKey, testCase.secretKey, t)
		...
		reqHeadBucket := signature.MustNewSignedV4Request(http.MethodHead, testCase.bucketName, 0, nil, "s3", testCase.accessKey, testCase.secretKey, t)
...
func TestS3ApiServer_ListBucketHandler(t *testing.T) {
	...
	for i, testCase := range testCases {
		...
		reqListBucket := signature.MustNewSignedV4Request(http.MethodGet, "/", 0, nil, "s3", testCase.accessKey, testCase.secretKey, t)
...
func TestS3ApiServer_DeleteBucketHandler(t *testing.T) {
	...
	for i, testCase := range testCases {
		...
		reqDeleteBucket := signature.MustNewSignedV4Request(http.MethodDelete, "/testbucketdel", 0,
			nil, "s3", testCase.accessKey, testCase.secretKey, t)
...
func TestS3ApiServer_BucketPolicyHandler(t *testing.T) {
	u := "/testbucketpoliy"
	reqPutBucket := signature.MustNewSignedV4Request(http.MethodPut, u, 0, nil, "s3", DefaultTestAccessKey, DefaultTestSecretKey, t)
	...
	reqPut := signature.MustNewSignedV4Request(http.MethodPut, u+"?policy", int64(len(p)), strings.NewReader(p),
		"s3", DefaultTestAccessKey, DefaultTestSecretKey, t)
	fmt.Println("put:", reqTest(reqPut).Body.String())

	reqGet := signature.MustNewSignedV4Request(http.MethodGet, u+"?policy", 0, nil, "s3",
		DefaultTestAccessKey, DefaultTestSecretKey, t)
	resp1 := policy.Policy{}
	json.Unmarshal([]byte(reqTest(reqGet).Body.String()), &resp1)
	fmt.Println("get:", resp1)

	reqDel := signature.MustNewSignedV4Request(http.MethodDelete, u+"?policy", 0, nil, "s3", DefaultTestAccessKey, DefaultTestSecretKey, t)
