...
//go run -tags example main.go adduser --addr=127.0.0.1:9985 --access-key=test --secret-key=test --username=wpg --pass=wpg123456
...
func add(addr, accessKey, secretKey, username, pass string) error {
	if addr == "" || accessKey == "" || secretKey == "" || username == "" || pass == "" {
		fmt.Println("please check your input\n " +
			"USAGE ERROR: go run -tags example main.go adduser --addr= --access-key= --secret-key= --username= --pass=")
