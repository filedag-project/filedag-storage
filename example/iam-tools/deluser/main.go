...
//go run -tags example main.go del-user --addr=127.0.0.1:9985 --access-key=test --secret-key=test --username=wpg

func main() {
	var addr, accessKey, secretKey, username string
	f := flag.NewFlagSet("del-user", flag.ExitOnError)
	...
	default:
		fmt.Println("expected 'del-user' subcommands")
		os.Exit(1)
	}
}
func del(addr, accessKey, secretKey, username string) error {
	if addr == "" || accessKey == "" || secretKey == "" || username == "" {
		fmt.Println("please check your input\n " +
			"USAGE ERROR: go run -tags example main.go del-user --addr= --access-key= --secret-key= --username= ")
