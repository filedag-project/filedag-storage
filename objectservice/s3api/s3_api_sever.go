...
type s3ApiServer struct {
	...
	stats   *httpstatss.HTTPStats
}

//registerS3Router Register APIs
func (s3a *s3ApiServer) registerS3Router(router *mux.Router, stats *httpstatss.HTTPStats) {
	// API Router
...
func (s3a *s3ApiServer) registerSTSRouter(router *mux.Router, stats *httpstatss.HTTPStats) {
	apiRouter := router.PathPrefix("/").Subrouter()
...
func NewS3Server(router *mux.Router, authSys *iam.AuthSys, bmSys *store.BucketMetadataSys, storageSys *store.StorageSys, stats *httpstatss.HTTPStats) {
