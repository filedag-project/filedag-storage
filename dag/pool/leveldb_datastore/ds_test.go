...
func addTestCases(t *testing.T, d *Datastore, testcases map[string]string) {
	for k, v := range testcases {
		dsk := ds.NewKey(k)
		if err := d.Put(testctx, dsk, []byte(v)); err != nil {
			t.Fatal(err)
		}
	}
...
func testQuery(t *testing.T, d *Datastore) {
	...
	rs, err := d.Query(testctx, dsq.Query{Prefix: "/a/"})
	if err != nil {
		t.Fatal(err)
	}
	...
	rs, err = d.Query(testctx, dsq.Query{Prefix: "/a/", Offset: 2, Limit: 2})
	if err != nil {
		t.Fatal(err)
	}
	...
	rs, err = d.Query(testctx, dsq.Query{Orders: []dsq.Order{dsq.OrderByKey{}}})
	if err != nil {
		t.Fatal(err)
	}
	...
	rs, err = d.Query(testctx, dsq.Query{Orders: []dsq.Order{dsq.OrderByKeyDescending{}}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestCloseRace(t *testing.T) {
	d, close := newDS(t)
	for n := 0; n < 100; n++ {
		d.Put(testctx, ds.NewKey(fmt.Sprintf("%d", n)), []byte(fmt.Sprintf("test%d", n)))
	}

	tx, _ := d.NewTransaction(testctx, false)
	tx.Put(testctx, ds.NewKey("txnversion"), []byte("bump"))
...
func TestCloseSafety(t *testing.T) {
	...
	tx, _ := d.NewTransaction(testctx, false)
	err := tx.Put(testctx, ds.NewKey("test"), []byte("test"))
	if err != nil {
...
func testBatching(t *testing.T, d *Datastore) {
	...
	for k, v := range testcases {
		err := b.Put(testctx, ds.NewKey(k), []byte(v))
		if err != nil {
			t.Fatal(err)
		}
	}

	err = b.Commit(testctx)
	if err != nil {
		t.Fatal(err)
	}

	for k, v := range testcases {
		val, err := d.Get(testctx, ds.NewKey(k))
		if err != nil {
			t.Fatal(err)
		}
...
func TestDiskUsage(t *testing.T) {
	...
	k := ds.NewKey("more")
	err = d.Put(testctx, k, []byte("value"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestTransactionCommit(t *testing.T) {
	...
	txn, err := d.NewTransaction(testctx, false)
	if err != nil {
		t.Fatal(err)
	}
	defer txn.Discard(testctx)

	if err := txn.Put(testctx, key, []byte("hello")); err != nil {
		t.Fatal(err)
	}
	if val, err := d.Get(testctx, key); err != ds.ErrNotFound {
		t.Fatalf("expected ErrNotFound, got err: %v, value: %v", err, val)
	}
	if err := txn.Commit(testctx); err != nil {
		t.Fatal(err)
	}
	if val, err := d.Get(testctx, key); err != nil || !bytes.Equal(val, []byte("hello")) {
...
func TestTransactionDiscard(t *testing.T) {
	...
	if txn.Discard(testctx); err != nil {
		t.Fatal(err)
	}
	if val, err := d.Get(testctx, key); err != ds.ErrNotFound {
		t.Fatalf("expected ErrNotFound, got err: %v, value: %v", err, val)
...
func TestTransactionManyOperations(t *testing.T) {
	...
	// Insert all entries.
	for i := 0; i < 5; i++ {
		if err := txn.Put(testctx, keys[i], []byte(fmt.Sprintf("hello%d", i))); err != nil {
			t.Fatal(err)
		}
	}

	// Remove the third entry.
	if err := txn.Delete(testctx, keys[2]); err != nil {
		t.Fatal(err)
	}

	// Check existences.
	if has, err := txn.Has(testctx, keys[1]); err != nil || !has {
		t.Fatalf("expected key[1] to be present, err: %v, has: %v", err, has)
	}
	if has, err := txn.Has(testctx, keys[2]); err != nil || has {
		t.Fatalf("expected key[2] to be absent, err: %v, has: %v", err, has)
	}

	var res dsq.Results
	if res, err = txn.Query(testctx, dsq.Query{Prefix: "/test"}); err != nil {
