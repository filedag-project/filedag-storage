...
func assertPinned(t *testing.T, p dppin.Pinner, c cid.Cid, failmsg string) {
...
func assertPinnedWithType(t *testing.T, p dppin.Pinner, c cid.Cid, mode dppin.Mode, failmsg string) {
	modeText, pinned, err := p.IsPinnedWithType(context.Background(), c, mode)
	if err != nil {
		t.Fatal(err)
	}

	expect, ok := dppin.ModeToString(mode)
	if !ok {
...
func assertUnpinned(t *testing.T, p dppin.Pinner, c cid.Cid, failmsg string) {
	_, pinned, err := p.IsPinned(context.Background(), c)
	if err != nil {
		t.Fatal(err)
	}
...
func TestPinnerBasic(t *testing.T) {
	...
	assertPinnedWithType(t, p, ak, dppin.Direct, "Expected direct pin")
	...
	assertPinnedWithType(t, p, bk, dppin.Recursive, "Recursively pinned node not found")
	...
	for _, pn := range pinned {
		switch pn.Key {
		case ak:
			if pn.Mode != dppin.Direct {
				t.Error("A pinned with wrong mode")
			}
		case bk:
			if pn.Mode != dppin.Recursive {
				t.Error("B pinned with wrong mode")
			}
		case ck:
			if pn.Mode != dppin.Indirect {
	...
	err = p.Unpin(ctx, dk, true)
	if err != dppin.ErrNotPinned {
		t.Fatal("expected error:", dppin.ErrNotPinned)
	}
...
func TestRemovePinWithMode(t *testing.T) {
	...
	ok, err := p.removePinsForCid(ctx, ak, dppin.Recursive)
	if err != nil {
		t.Fatal(err)
	}
	if ok {
	...
	p.RemovePinWithMode(ak, dppin.Direct)
...
func TestFlush(t *testing.T) {
	...
	p.PinWithMode(k, dppin.Recursive)
...
func TestLoadDirty(t *testing.T) {
	...
	has, err = p.removePinsForCid(ctx, bk, dppin.Any)
	if err != nil {
		t.Fatal(err)
	}
	if !has {
...
func TestEncodeDecodePin(t *testing.T) {
	...
	pin := newPin(c, dppin.Recursive, "testpin")
	...
	if decPin.Mode != pin.Mode {
		expect, _ := dppin.ModeToString(pin.Mode)
		got, _ := dppin.ModeToString(decPin.Mode)
...
func makeTree(ctx context.Context, aBranchLen int, dserv ipld.DAGService, p dppin.Pinner) (aKeys []cid.Cid, bk cid.Cid, ck cid.Cid, err error) {
	if aBranchLen < 3 {
...
func pinNodes(nodes []ipld.Node, p dppin.Pinner, recursive bool) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	var err error
...
func unpinNodes(nodes []ipld.Node, p dppin.Pinner) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	var err error
...
func benchmarkNthPin(b *testing.B, count int, pinner dppin.Pinner, dserv ipld.DAGService) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
...
func benchmarkNPins(b *testing.B, count int, pinner dppin.Pinner, dserv ipld.DAGService) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	nodes := makeNodes(count, dserv)
	b.ResetTimer()
...
func benchmarkNUnpins(b *testing.B, count int, pinner dppin.Pinner, dserv ipld.DAGService) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	nodes := makeNodes(count, dserv)
	pinNodes(nodes, pinner, true)
...
func benchmarkPinAll(b *testing.B, count int, pinner dppin.Pinner, dserv ipld.DAGService) {
	nodes := makeNodes(count, dserv)
	b.ResetTimer()
...
func TestCidIndex(t *testing.T) {
	...
	// Pin the cid
	pid, err := pinner.addPin(ctx, c, dppin.Recursive, "")
	if err != nil {
		t.Fatal(err)
	}
