...
func init() {
	directStr, ok := dppin.ModeToString(dppin.Direct)
	if !ok {
		panic("could not find Direct pin enum")
	}
	linkDirect = directStr

	recursiveStr, ok := dppin.ModeToString(dppin.Recursive)
	if !ok {
...
func newPin(c cid.Cid, mode dppin.Mode, name string) *pin {
	return &pin{
...
func (p *pinner) Pin(ctx context.Context, node ipld.Node, recurse bool) error {
	...
	if recurse {
		...
		if found {
			_, err = p.removePinsForCid(ctx, c, dppin.Direct)
			if err != nil {
				return err
			}
		}

		_, err = p.addPin(ctx, c, dppin.Recursive, "")
		if err != nil {
			return err
		}
	} else {
		...
		_, err = p.addPin(ctx, c, dppin.Direct, "")
		if err != nil {
			return err
		}
	}
...
func (p *pinner) addPin(ctx context.Context, c cid.Cid, mode dppin.Mode, name string) (string, error) {
...
func (p *pinner) IsPinned(ctx context.Context, c cid.Cid) (string, bool, error) {
	...
	return p.isPinnedWithType(ctx, c, dppin.Any)
}
...
func (p *pinner) IsPinnedWithType(ctx context.Context, c cid.Cid, mode dppin.Mode) (string, bool, error) {
	p.lock.RLock()
	defer p.lock.RUnlock()
...
func (p *pinner) isPinnedWithType(ctx context.Context, c cid.Cid, mode dppin.Mode) (string, bool, error) {
	...
	default:
		err := fmt.Errorf(
			...
			mode, dppin.Direct, dppin.Indirect, dppin.Recursive,
			dppin.Internal, dppin.Any)
		return "", false, err
	}
...
func (p *pinner) CheckIfPinned(ctx context.Context, cids ...cid.Cid) ([]dppin.Pinned, error) {
	pinned := make([]dppin.Pinned, 0, len(cids))
	toCheck := cid.NewSet()
	...
	for _, c := range cids {
		...
		if has {
			pinned = append(pinned, dppin.Pinned{Key: c, Mode: dppin.Recursive})
		} else {
			has, err = p.cidDIndex.HasAny(ctx, cidKey)
			if err != nil {
				return nil, err
			...
			if has {
				pinned = append(pinned, dppin.Pinned{Key: c, Mode: dppin.Direct})
			} else {
				toCheck.Add(c)
			}
		}
	...
	err := p.cidRIndex.ForEach(ctx, "", func(key, value string) bool {
		...
		e = merkledag.Walk(ctx, merkledag.GetLinksWithDAG(p.dserv), rk, func(c cid.Cid) bool {
			...
			if toCheck.Has(c) {
				pinned = append(pinned, dppin.Pinned{Key: c, Mode: dppin.Indirect, Via: rk})
				toCheck.Remove(c)
			}
	...
	for _, k := range toCheck.Keys() {
		pinned = append(pinned, dppin.Pinned{Key: k, Mode: dppin.NotPinned})
	}
...
// be successful.
func (p *pinner) RemovePinWithMode(c cid.Cid, mode dppin.Mode) {
	ctx := context.TODO()
	// Check cache to see if CID is pinned
	switch mode {
	case dppin.Direct, dppin.Recursive:
	default:
...
func (p *pinner) removePinsForCid(ctx context.Context, c cid.Cid, mode dppin.Mode) (bool, error) {
	...
	for _, pid := range ids {
		...
		if mode == dppin.Any || pp.Mode == mode {
...
func (p *pinner) Update(ctx context.Context, from, to cid.Cid, unpin bool) error {
	...
	_, err = p.addPin(ctx, to, dppin.Recursive, "")
	if err != nil {
		return err
	}

	if unpin {
		_, err = p.removePinsForCid(ctx, from, dppin.Recursive)
		if err != nil {
			return err
		}
	}
...
// counts
func (p *pinner) PinWithMode(c cid.Cid, mode dppin.Mode) {
	ctx := context.TODO()
