...
function test_object_download() {
  ...
  test_except "test download txt no permission" 1 $?
  mc cat loc1/testadownload/test_cp_no_exist.txt >/dev/null 2>&1 # no exist
  test_except "test download txt no exist" 1 $?
  echo " 2)admin download file"
  ...
  test_except "test admin download txt no exist" 1 $?
}
